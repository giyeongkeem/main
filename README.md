...
이 저장소에는 두 가지 서비스가 있습니다.

1. **[섹터 뉴스 리포트 에이전트](#섹터-뉴스-리포트-에이전트)** — 관심 주식 섹터 데일리 리포트 생성
2. **[전자세금계산서 발급 서비스](#전자세금계산서-발급-서비스)** — 세금계산서 작성·국세청 발급 웹 서비스

---

# 전자세금계산서 발급 서비스

세금계산서를 웹에서 작성·관리하고 국세청에 발급하는 서비스입니다.
...
- **세금계산서 작성** — 필수 기재사항 검증(사업자등록번호 체크섬 포함), 품목별 공급가액·세액 자동 계산(과세 10%, 원 미만 절사), 과세/영세/면세·영수/청구 지원
- **공급자(내 사업자) 정보 저장** — 설정 탭에 한 번 입력하면 작성 시 자동 채움
- **거래처 자동 등록** — 발행 이력의 공급받는자가 거래처로 저장되어 다음 작성 때 선택만 하면 자동 입력
- **발급 목록 관리** — 임시저장 → 발급완료 → 국세청 전송완료 상태 추적, 국세청 승인번호 표시

## 국세청 발급 경로 (둘 중 선택)

국세청은 개별 서비스의 직접 전송을 허용하지 않으므로(인증 ASP만 가능), 두 가지 경로를 지원합니다.

**① 팝빌(Popbill) API 자동 발급** — 버튼 클릭으로 발급 + 국세청 전송까지 자동 처리

1. [popbill.com](https://www.popbill.com) 가입 → 연동신청 → LinkID/SecretKey 발급
2. `pip install popbill`
3. 환경변수 설정 후 재시작:
   ```
   POPBILL_LINK_ID=발급받은_LinkID
   POPBILL_SECRET_KEY=발급받은_SecretKey
   POPBILL_IS_TEST=true      # 테스트베드(모의 발급). 운영 전환 시 false
   ```
4. 웹 UI에서 **저장 후 국세청 발급** 클릭 → 발급·전송, 목록에서 승인번호 확인

> ⚠️ `POPBILL_IS_TEST=false`(운영)에서는 **실제 국세청 신고가 이루어집니다.** 반드시 테스트베드로 충분히 검증한 뒤 전환하세요.

**② 홈택스 수동 발급** — API 연동 없이 사용 (기본 모드)

1. 세금계산서를 임시저장으로 쌓은 뒤 **발급 목록 → 홈택스 일괄발급 CSV** 내려받기
2. [홈택스](https://hometax.go.kr) → 전자(세금)계산서 → **일괄발급** 메뉴에서 공식 엑셀 양식을 내려받아 CSV 데이터 행을 붙여넣고 업로드
3. 건별 **국세청 표준(v3.0) 형식 XML**도 내려받을 수 있습니다(보관·타 시스템 연계용, 전자서명 미포함)

## 배포·기타

- 공개 URL로 배포할 때는 `APP_PASSWORD` 환경변수를 반드시 설정하세요(HTTP Basic 인증). 사업자 정보가 저장됩니다.
- 데이터는 SQLite 파일(`tax_invoices.db`, `TAX_INVOICE_DB`로 변경 가능)에 저장됩니다.
- 테스트: `python tests/test_tax_invoice_service.py`
