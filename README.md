...
`run.sh`는 주소와 함께 **접속 비밀번호**를 출력합니다. LAN에 열리는 만큼(0.0.0.0 바인딩) 무단 접속을 막기 위해 비밀번호가 자동 생성되어 `.env`에 저장됩니다. 접속 시 사용자 이름은 아무거나, 비밀번호는 출력된 값을 넣습니다.

로그인 → 주제 입력 → 생성 → 완료되면 미리보기·다운로드. 아이폰 Safari에서 공유 버튼 → "홈 화면에 추가"를 하면 앱처럼 쓸 수 있습니다.

> 처음 실행 시 macOS가 "들어오는 네트워크 연결을 허용하시겠습니까?"라고 물으면 **허용**을 눌러야 아이폰에서 접속됩니다. 비밀번호 없이 맥에서만 쓸 거라면 `uvicorn app.main:app --port 8000`으로 실행하세요 (127.0.0.1 전용, 외부 접속 차단).
>
> **Linux에서 실행 시:** 자막 폰트로 `Noto Sans CJK KR`이 필요합니다 — `sudo apt install fonts-noto-cjk`. 없으면 한국어 자막이 □로 깨집니다. macOS는 기본 폰트로 동작합니다.
